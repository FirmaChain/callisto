...
func (suite *DbTestSuite) TestBigDipperDb_SaveStakingPool() {
	// Save the data
	original := types.NewPool(math.NewInt(50), math.NewInt(100), 10)
	...
	pool := types.NewPool(math.NewInt(1), math.NewInt(1), 8)
	...
	pool = types.NewPool(math.NewInt(1), math.NewInt(1), 10)
	err = suite.database.SaveStakingPool(pool)
	suite.Require().NoError(err)
	...
	pool = types.NewPool(math.NewInt(1000000), math.NewInt(1000000), 20)
	err = suite.database.SaveStakingPool(pool)
	suite.Require().NoError(err)
